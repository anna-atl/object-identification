...
def calculate_weighted_matches_ratios(buckets_of_bands, docs_shingled, shingles_weights_in_docs, docs_mapping_old_new):
    matched_pairs = {} #keys - tuple of duplicate docs and values - jacc of docs(lists of shingles(numbers))
    for buckets_of_band in buckets_of_bands:
        for bucket, docs_in_bucket in buckets_of_band.items(): #values_list - doc indexes in one buckets
            for doc_index_1 in docs_in_bucket: #iterating through doc_indexes
                for doc_index_2 in docs_in_bucket:
                    if doc_index_2 > doc_index_1 and (doc_index_1, doc_index_2) not in matched_pairs:
                        try:
                            matched_pairs.setdefault((doc_index_1, doc_index_2), []).append(gen_jaccard_comparison(docs_shingled[docs_mapping_old_new[doc_index_1]], docs_shingled[docs_mapping_old_new[doc_index_2]], shingles_weights_in_docs[docs_mapping_old_new[doc_index_1]], shingles_weights_in_docs[docs_mapping_old_new[doc_index_2]]))
                        except:
                            print('didnt work')
...
def main(buckets_of_bands, docs_shingled, comparison_method, attribute_weight, shingles_weights_in_docs, matching_attribute, docs_mapping_old_new):

    start_time = time.time()
    print("----Started calculating comparison functions for potential matches in buckets...")
    if comparison_method == 'weighted jaccard':
        matched_pairs = calculate_weighted_matches_ratios(buckets_of_bands, docs_shingled, shingles_weights_in_docs, docs_mapping_old_new)
